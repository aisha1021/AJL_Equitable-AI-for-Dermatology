...
weights_path = '/kaggle/input/pretrained-model-weights/resnet50_weights_tf_dim_ordering_tf_kernels_notop.h5'

# Load Pretrained ResNet50 Model (With Custom Weights, Without Top Layers)
...
x = Dropout(0.5)(x)
output_layer = Dense(21, activation='softmax')(x)  # Adjust number of classes
...
# Save history
with open('resnet50-history.pkl', 'wb') as f:
...
model = load_model("/kaggle/input/resnet50-model-dataset/resnet50_model.h5")

# Unfreeze last 10 layers of the loaded model
...
# Save history
with open('resnet50-finetuned-history.pkl', 'wb') as f:
    pickle.dump(history_fine.history, f)

# Load history
with open('resnet50-finetuned-history.pkl', 'rb') as f:
...
# Save the entire model, including weights, architecture, and optimizer state
model.save('resnet50_model_finetuned.h5')
...
# Save to CSV
submission_file = "submission_resnet50_finetuned.csv"
