...
# Define input shape for DenseNet121
input_shape = (224, 224, 3)  # DenseNet121 uses 224x224 input size
input_layer = Input(shape=input_shape, name="input_layer")

weights = '/kaggle/input/desnsenet121-weights/densenet121_weights_tf_dim_ordering_tf_kernels_notop.h5'

# Load DenseNet121 without top layers
base_model = DenseNet121(weights=weights, include_top=False, input_tensor=input_layer)
...
model.save('desnsenet121_model.h5')

# Save history
with open('desnsenet121-history.pkl', 'wb') as f:
    pickle.dump(history.history, f)

# Load history
with open('desnsenet121-history.pkl', 'rb') as f:
...
model = load_model("/kaggle/input/densenet121-model/desnsenet121_model.h5")

# Unfreeze last 30 layers of the loaded model
...
# Save history
with open('desnsenet121-finetuned-history.pkl', 'wb') as f:
    pickle.dump(history_fine.history, f)

# Load history
with open('desnsenet121-finetuned-history.pkl', 'rb') as f:
...
model.save('desnsenet121_finetuned_model.h5')

"""## 7. Make Predictions on Test Data"""

# Load the pre-trained model (with frozen layers)
from tensorflow.keras.models import load_model
model = load_model("/kaggle/working/desnsenet121_finetuned_model.h5")
...
predictions = model.predict(test_generator) # change this after all three models have run
